...
export class Migration20250205180751 extends Migration {
  override up(): void {
    this.addSql(
      `create table "loans" ("id" uuid not null default gen_random_uuid(), "applicant_name" varchar(255) not null, "requested_amount" int not null, "status" text check ("status" in ('PENDING', 'APPROVED', 'REJECTED')) not null default 'PENDING', "created_at" timestamptz not null, "updated_at" timestamptz not null, constraint "loans_pkey" primary key ("id"));`,
    );
  }
